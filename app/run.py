...
Original file is located at
    https://colab.research.google.com/drive/1Lz64htmKD4iS3GNhS7iR4VLpK3_BfdGY
"""

!pip install folium geopandas shapely streamlit_folium
!pip install ployly
...
state_rows=pd.read_csv('/content/tat.csv')
df1=pd.read_csv('/content/superstore.csv')
df=pd.read_csv('/content/state_reading.csv')
model2=pickle.load(open('/content/state_prediction.pkl','rb'))
stop_but=0
input_values=[]
selected_state=[]
prediction=[]
...
def load_df():
    walmartStores = pd.read_csv('/content/walmart-stores.csv')
    # data = {'ID': ['Monkey', 'B'],
    #         'Icon_ID': [0, 1],
    #         'Icon_Size': [50,50],
    #         'Opacity': [1, 1],
...
def load_Citydf():
    cityData = pd.read_csv('/content/cityData.csv')
...
def main():
  ...
  tab1,tab2=st.tabs(['Prediction','Long&Lats'])

  with tab1:
    st.title('NextBestLocation')
    pop_slider=float(st.slider(label='Population',min_value=10000,max_value=10000000))
    road_factor=float(st.slider(label="Road Quality",min_value=10000,max_value=1000000))
    economy=st.slider(label="Economy Index",min_value=1000,max_value=1000000)
    literacy_rate=st.slider(label="Literacy Rate",min_value=0,max_value=10)
    tier_value=st.selectbox('Tier Place you looking for?',('Top','Intermeditate','Low'))
    ...
    if tier_value=='Top':
      ...
      tier_value=3
    lst=[pop_slider,road_factor,tier_value,economy,literacy_rate]
    ...
    if st.button('Predict',keys='same'):
      ...
      for seconds in range(7):
        ...
        df1['profit_new']=df1['Profit']*np.random.choice(range(1,5))
        sales=np.mean(df1['sales_new'])
        proft=np.mean(df1['proft_new'])

        with placeholder.container():
          kpi1,kpi2=st.columns(2)
          kpi1.metric(label='Sales in Thousands',value=round(sales),delta=round(sales)-10)
          kpi2.metric(label='Profit in Thousands',value=round(proft),delta=round(proft-10,2))
          time.sleep(0.5)
  ...
  with tab2:
    ...
    with map_col:
      level1_map_data=st.folium(m,height=700,width=1024)
